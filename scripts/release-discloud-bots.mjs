...
run("node", ["scripts/prepare-discloud-package.mjs"], {
  env: {
    ...
    DEV_BOT_RUNTIME_RECONCILE_INTERVAL_MS: process.env.DEV_BOT_RUNTIME_RECONCILE_INTERVAL_MS?.trim() || "60000",
    DEV_BOT_START_CONCURRENCY: process.env.DEV_BOT_START_CONCURRENCY?.trim() || "2",
    DEV_BOT_START_STAGGER_MS: process.env.DEV_BOT_START_STAGGER_MS?.trim() || "30000",
    DEV_BOT_NODE_MAX_OLD_SPACE_MB: process.env.DEV_BOT_NODE_MAX_OLD_SPACE_MB?.trim() || "128"
  }
});
