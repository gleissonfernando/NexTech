...
const AUTOMATED_CHANNEL_LABELS: Record<keyof AutomatedLogSettings["enabledChannels"], string> = {
  ...
  punishment: "🚨 Logs de punição",
  site: "🌐 Logs do site",
  verification: "🛡️ Verificação DC"
};
        ...
        <Card>
          <CardHeader>
            <div className="flex items-start justify-between gap-3">
              <div>
                <CardTitle>Sistema de Logs do Discord</CardTitle>
                <CardDescription>
                  Escolha quais eventos o bot deve registrar. Cada módulo marcado mantém seu canal em Logs Geral; módulos desmarcados não criam canal nem recebem eventos.
                </CardDescription>
              </div>
              <Switch checked={automated.enabled} disabled={disabled} onCheckedChange={(enabled) => { setAutomated((current) => current ? { ...current, enabled } : current); if (enabled) updateDraft("discordLogsEnabled", true); }} />
            </div>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              <p className="mb-1 text-sm font-medium text-zinc-200">Módulos de log ativos</p>
              <p className="mb-2 text-xs text-zinc-500">A configuração define o que será coletado; a estrutura automática cria, remove ou reutiliza somente os canais necessários.</p>
            ...
            <div>
              <p className="mb-1 text-sm font-medium text-zinc-200">Cargos autorizados a ver logs</p>
              <p className="mb-2 text-xs text-zinc-500">@everyone fica sem acesso; somente os cargos selecionados recebem permissão de visualização nos canais ativos.</p>
