...
export async function listVisibleModeUsers(guildId: string, botId: string, q?: string) { const { data } = await api.get<{ users: import("../types").VisibleModeUser[] }>(`/visible-mode/${guildId}/users`, { params: { ...botParams(botId), ...(q ? { q } : {}) } }); return data.users; }
export async function addVisibleModeUser(guildId: string, botId: string, userId: string) { const { data } = await api.put<{ user: import("../types").VisibleModeUser }>(`/visible-mode/${guildId}/users/${userId}`, null, { params: botParams(botId) }); return data.user; }
export async function removeVisibleModeUser(guildId: string, botId: string, userId: string) { const { data } = await api.delete<{ removed: boolean }>(`/visible-mode/${guildId}/users/${userId}`, { params: botParams(botId) }); return data; }
export async function clearVisibleModeUsers(guildId: string, botId: string) { const { data } = await api.delete<{ removed: number }>(`/visible-mode/${guildId}/users`, { params: botParams(botId) }); return data; }
