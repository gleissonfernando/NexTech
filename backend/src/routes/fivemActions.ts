...
fivemActionsRouter.post("/bot/:guildId/:architecture/actions", requireBot, async (req, res, next) => { try { const { guildId, architecture } = params(req); const botId = await botRuntimeId(req); await licensed(botId, architecture); res.status(201).json({ action: await saveFivemActionDefinition(botId, guildId, architecture, null, actionSchema.parse(req.body), req.get("x-actor-id") ?? "bot-runtime") }); } catch (error) { next(error); } });
fivemActionsRouter.patch("/bot/:guildId/:architecture/actions/:actionId", requireBot, async (req, res, next) => { try { const { guildId, architecture } = params(req); const botId = await botRuntimeId(req); await licensed(botId, architecture); res.json({ action: await saveFivemActionDefinition(botId, guildId, architecture, req.params.actionId!, actionSchema.partial().parse(req.body), req.get("x-actor-id") ?? "bot-runtime") }); } catch (error) { next(error); } });
fivemActionsRouter.delete("/bot/:guildId/:architecture/actions/:actionId", requireBot, async (req, res, next) => { try { const { guildId, architecture } = params(req); const botId = await botRuntimeId(req); await licensed(botId, architecture); const action = await deleteFivemActionDefinition(botId, guildId, architecture, req.params.actionId!); if (!action) throw routeError("Ação não encontrada.", 404); res.json({ action }); } catch (error) { next(error); } });
