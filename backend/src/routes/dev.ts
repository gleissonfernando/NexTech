...
const nexTechInviteSchema = z.object({
  ...
  backgroundEffect: z.enum(["fixed", "parallax", "zoom", "blur"]).nullable().optional(),
  ...
  landingPageTheme: z.enum(["discord", "nextech", "dark", "neon", "cyber", "gamer", "premium", "minimal", "modern"]).nullable().optional(),
  logChannelId: z.string().regex(/^\d{5,32}$/).nullable().optional().or(z.literal("")),
  logoUrl: z.string().url().max(2048).nullable().optional().or(z.literal("")),
  maxUses: z.number().int().min(1).max(100000).nullable().optional(),
  name: z.string().min(2).max(120),
  notes: z.string().max(800).nullable().optional().or(z.literal("")),
  overlayStyle: z.enum(["black", "blue", "red", "gradient", "none"]).nullable().optional(),
  particleStyle: z.enum(["none", "dots", "sparks", "neon", "smoke", "lines", "stars", "hex"]).nullable().optional(),
  panelChannelId: z.string().regex(/^\d{5,32}$/).nullable().optional().or(z.literal("")),
  ...
  showInviteCode: z.boolean().optional(),
  showMemberCount: z.boolean().optional(),
  showOnlineCount: z.boolean().optional(),
  showServerDescription: z.boolean().optional(),
  showServerName: z.boolean().optional(),
  showVerificationBadges: z.boolean().optional(),
