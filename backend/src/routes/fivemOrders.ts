...
async function canRead(req: Request, guildId: string, botId: string | null) {
  ...
  return (await canReadDevBotModule(req.res?.locals.dashboardAuth.user, botId, guildId, "fivem-washing"))
...
async function canManage(req: Request, guildId: string, botId: string | null) {
  ...
  return (await canUseDevBotModule(req.res?.locals.dashboardAuth.user, botId, guildId, "fivem-washing"))
...
async function assertRuntime(botId: string | null, guildId: string) {
  ...
  throw Object.assign(new Error(washingAccess.reason), { statusCode: 403 });
}
