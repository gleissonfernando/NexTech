...
async function scoped(req: Parameters<typeof resolveRequestBotId>[0], rawGuildId: unknown) { const parsedGuild = guildId.parse(rawGuildId); const botId = await resolveRequestBotId(req); if (!botId) throw error("Escopo do bot é obrigatório.", 400); const permissions = await getBotApiPermissions(botId); if (!permissions?.enabledModules.includes("temporary-voice")) throw error("Calls temporárias não estão ativadas para este bot.", 403); const authorization = await authorizeBotRuntimeModule({ botId, guildId: parsedGuild, moduleId: "temporary-voice" }); if (!authorization.allowed) throw error(translateAuthorizationReason(authorization.reason), 403); return { botId, guildId: parsedGuild }; }
function error(message: string, statusCode: number) { return Object.assign(new Error(message), { statusCode }); }
function translateAuthorizationReason(reason: string) {
  if (/not enabled|disabled/i.test(reason)) return "Calls temporárias não estão ativadas para este bot neste servidor.";
  if (/permission|allowed|authorized|liberado/i.test(reason)) return "Este bot não tem autorização para usar calls temporárias neste servidor.";
  return reason;
}
