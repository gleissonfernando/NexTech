...
const DEV_BOT_START_CONCURRENCY = env.DEV_BOT_START_CONCURRENCY ?? (env.NODE_ENV === "production" ? 4 : 3);
...
const DEV_BOT_START_STAGGER_MS = env.DEV_BOT_START_STAGGER_MS ?? (env.NODE_ENV === "production" ? 5_000 : 2_000);
...
export async function stopDevBotProcess(botId: string, options: StopDevBotOptions = {}) {
  ...
  if (shouldEmitDevBotShutdownSignal(Boolean(runtime), notifyBot)) {
...
export function shouldEmitDevBotShutdownSignal(runtimeExists: boolean, notifyBot: boolean) {
  return runtimeExists && notifyBot;
}
