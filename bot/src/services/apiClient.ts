...
export type PoliceRecruitmentQuestionType = "TEXT" | "LONG_TEXT" | "NUMBER" | "DATE" | "TIME" | "USER_SELECT" | "ROLE_SELECT" | "SELECT" | "MULTI_SELECT" | "BOOLEAN";
...
export type PoliceQruSettings = { id: string; botId: string; guildId: string; enabled: boolean; recordChannelId: string | null; logChannelId: string | null; approvalChannelId: string | null; temporaryCategoryId: string | null; allowedRoleIds: string[]; supervisorRoleIds: string[]; teamRoleId: string | null; deleteChannelSeconds: number; color: string; panelTitle: string; panelDescription: string; panelImageUrl: string | null; panelMessage: string; rankingChannelId: string | null; rankingMessageId: string | null; rankingResetAt: string | null };
export type PoliceQruMedia = { originalUrl: string; resolvedUrl?: string | null; storedUrl?: string | null; mediaId?: string | null; mimeType?: string | null; fileName?: string | null; fileSize?: number | null; sha256?: string | null; status: "processing" | "ready" | "failed"; error?: string | null };
export type PoliceQruRecord = { id: string; botId: string; guildId: string; boNumber: string; qruType: string; occurrenceDate: string; evidenceUrl: string; media?: PoliceQruMedia[]; seizures: string | null; notes: string | null; vehicle: string | null; status: "pending" | "approved" | "rejected"; authorId: string; authorName: string; officers: PoliceQruOfficer[]; temporaryChannelId: string | null; recordChannelId: string | null; recordMessageId: string | null; approvalChannelId: string | null; approvalMessageId: string | null; approvedById: string | null; approvedByName: string | null; approvedAt: string | null; rejectionCount: number; rejections: Array<{ reason: string; rejectedAt: string; supervisorId: string; supervisorName: string }>; createdAt: string; updatedAt: string };
...
export type PolicePromotionSettings = { id: string; botId: string; guildId: string; enabled: boolean; approverRoleIds: string[]; defaultApprovalChannelId: string | null; defaultCategoryId: string | null; defaultHistoryChannelId: string | null; defaultLogChannelId: string | null; defaultPanelChannelId: string | null; instructorRoleIds: string[]; promotions: PolicePromotionDefinition[]; quarantineRoleIds: string[]; rejecterRoleIds: string[]; createdAt: string; updatedAt: string };
...
export type PoliceRankUpRank = { id: string; allowSkip: boolean; allowedPreviousRanks: string[]; createdAt: string | Date; description: string | null; emoji: string | null; enabled: boolean; hierarchyPosition: number; name: string; roleId: string; updatedAt: string | Date };
export type PoliceRankUpPermission = "view" | "approve" | "reject" | "cancel" | "manage_ranks" | "manage_channels" | "publish_panel" | "view_logs" | "manage_responsibles";
export type PoliceRankUpSettings = { id: string; adminChannelId: string | null; adminRoleIds: string[]; adminUserIds: string[]; allowRequesterCancel: boolean; approvedDeleteSeconds: number; autoDeleteChannels: boolean; blockDemotions: boolean; blockMultipleRanks: boolean; botId: string; createdAt: string; enabled: boolean; guildId: string; logChannelId: string | null; mentionResponsibles: boolean; minRequestIntervalHours: number; notifyByDm: boolean; onlyNextRank: boolean; panelChannelId: string | null; panelMessage: string; panelMessageId: string | null; permissions: { roles: Record<string, PoliceRankUpPermission[]>; users: Record<string, PoliceRankUpPermission[]> }; ranks: PoliceRankUpRank[]; rejectedDeleteSeconds: number; requireApprovalForInitialRank: boolean; responsibleRoleIds: string[]; responsibleUserIds: string[]; temporaryCategoryId: string | null; temporaryChannelName: string; updatedAt: string; updatedBy: string | null };
export type PoliceRankUpRequest = { id: string; botId: string; completedAt: string | null; createdAt: string; currentRankId: string | null; currentRoleId: string | null; errorReason: string | null; guildId: string; messageId: string | null; protocol: string; requestedRankId: string; requestedRoleId: string; reviewReason: string | null; reviewedAt: string | null; reviewedBy: string | null; reviewedByName: string | null; status: "pending" | "approved" | "rejected" | "cancelled" | "error"; temporaryChannelId: string | null; tenantId: string; updatedAt: string; userDisplayName: string; userId: string; username: string };
...
export type FivemGoalSubmission = { correctionRequestId: string | null; createdAt: string; description: string | null; id: string; metaId: string; periodId: string | null; proofUrl: string | null; registeredAt: string | null; registrationId: string | null; replacementForRegistrationId: string | null; status: "pending" | "approved" | "refused" | "correction_requested"; userId: string; value: number };
...
export class ApiClient {
  ...
  async recordFactionChestMovement(guildId: string, input: { action: "add" | "remove"; actorId: string; actorName: string; bauId?: string | null; channelId?: string | null; item?: string; items?: string | null; messageId?: string | null; quantity?: number; reason?: string | null }) {
    const { data } = await this.http.post<{ item: FactionChestItem | null; items: FactionChestItem[]; log: FactionChestLog | null; logs: FactionChestLog[]; operationCode: string }>(`/faction-chests/bot/${guildId}/movements`, input);
    return data;
  }
  ...
  async getPoliceRecruitmentSettings(guildId: string) { const { data } = await this.http.get<{ settings: PoliceRecruitmentSettings }>(`/police-recruitment/bot/${guildId}/settings`); return data.settings; }
  async savePoliceRecruitmentSettings(guildId: string, input: Partial<PoliceRecruitmentSettings>, actorId?: string | null) { const { data } = await this.http.patch<{ settings: PoliceRecruitmentSettings }>(`/police-recruitment/bot/${guildId}/settings`, input, { headers: actorId ? { "x-actor-id": actorId } : undefined }); return data.settings; }
  async listPoliceRecruitmentQuestions(guildId: string) { const { data } = await this.http.get<{ questions: PoliceRecruitmentQuestion[] }>(`/police-recruitment/bot/${guildId}/questions`); return data.questions; }
  async createPoliceRecruitmentSession(input: { guildId: string; recruiter: { avatar: string | null; discordId: string; displayName: string; policeId?: string | null; username: string } }) { const { data } = await this.http.post<{ session: PoliceRecruitmentSession }>("/police-recruitment/bot/sessions", input); return data.session; }
  async setPoliceRecruitmentSessionChannel(sessionId: string, channelId: string, panelMessageId: string) { const { data } = await this.http.patch<{ session: PoliceRecruitmentSession }>(`/police-recruitment/bot/sessions/${sessionId}/channel`, { channelId, panelMessageId }); return data.session; }
  async selectPoliceRecruitmentUser(sessionId: string, input: { actorId: string; avatar: string | null; discordId: string; displayName: string; username: string }) { const { data } = await this.http.post<{ session: PoliceRecruitmentSession }>(`/police-recruitment/bot/sessions/${sessionId}/recruited`, input); return data.session; }
  async savePoliceRecruitmentAnswer(sessionId: string, input: { actorId: string; move?: "stay" | "next" | "previous"; questionId: string; value: string | string[] | boolean | number | null }) { const { data } = await this.http.post<{ session: PoliceRecruitmentSession }>(`/police-recruitment/bot/sessions/${sessionId}/answers`, input); return data.session; }
  async movePoliceRecruitmentQuestion(sessionId: string, input: { actorId: string; direction: "next" | "previous" }) { const { data } = await this.http.post<{ session: PoliceRecruitmentSession }>(`/police-recruitment/bot/sessions/${sessionId}/move`, input); return data.session; }
  async finishPoliceRecruitmentSession(sessionId: string, actorId: string) { const { data } = await this.http.post<{ report: PoliceRecruitmentReport }>(`/police-recruitment/bot/sessions/${sessionId}/finish`, { actorId }); return data.report; }
  async cancelPoliceRecruitmentSession(sessionId: string, actorId: string, status?: "CANCELLED" | "EXPIRED") { const { data } = await this.http.post<{ session: PoliceRecruitmentSession }>(`/police-recruitment/bot/sessions/${sessionId}/cancel`, { actorId, status }); return data.session; }
  async getPoliceRecruitmentSession(sessionId: string) { const { data } = await this.http.get<{ session: PoliceRecruitmentSession }>(`/police-recruitment/bot/sessions/${sessionId}`); return data.session; }
  async getPoliceRecruitmentSessionByChannel(channelId: string) { const { data } = await this.http.get<{ session: PoliceRecruitmentSession | null }>(`/police-recruitment/bot/sessions/channel/${channelId}`); return data.session; }
  async listPoliceRecruitmentExpiredSessions() { const { data } = await this.http.get<{ sessions: PoliceRecruitmentSession[] }>("/police-recruitment/bot/sessions/expired/list"); return data.sessions; }
  async getPoliceRecruitmentRecruiter(guildId: string, discordId: string) { const { data } = await this.http.get<{ recruiter: PoliceRecruiter | null }>(`/police-recruitment/bot/${guildId}/recruiters/${discordId}`); return data.recruiter; }
  async savePoliceRecruitmentRecruiterForumThread(guildId: string, discordId: string, input: { displayName?: string | null; forumThreadId: string; policeId?: string | null }) { const { data } = await this.http.put<{ recruiter: PoliceRecruiter | null }>(`/police-recruitment/bot/${guildId}/recruiters/${discordId}/forum-thread`, input); return data.recruiter; }
  async listPoliceRecruitmentReports(guildId: string, params?: { q?: string | null; recruitedDiscordId?: string | null; recruiterDiscordId?: string | null }) { const { data } = await this.http.get<{ reports: PoliceRecruitmentReport[] }>(`/police-recruitment/bot/${guildId}/reports`, { params }); return data.reports; }
  async updatePoliceRecruitmentReportPublication(reportId: string, input: { forumMessageId: string | null; forumThreadId: string }) { const { data } = await this.http.patch<{ report: PoliceRecruitmentReport }>(`/police-recruitment/bot/reports/${reportId}/publication`, input); return data.report; }
  ...
  async savePoliceQruSettings(guildId: string, input: Partial<Pick<PoliceQruSettings, "rankingChannelId" | "rankingMessageId" | "rankingResetAt">>) { const { data } = await this.http.patch<{ settings: PoliceQruSettings }>(`/police-qru/bot/${guildId}/settings`, input); return data.settings; }
  ...
  async assignPolicePromotionEvaluator(requestId: string, input: { evaluatorId: string; evaluatorName: string; evaluatorRoleIds?: string[] }) { const { data } = await this.http.post<{ request: PolicePromotionRequest }>(`/police-promotions/bot/requests/${requestId}/assign`, input); return data.request; }
  async finishPolicePromotionEvaluation(requestId: string, input: { evaluatorId: string; evaluatorRoleIds?: string[]; evaluationNotes: string; evaluationResult: "approved" | "rejected" }) { const { data } = await this.http.post<{ request: PolicePromotionRequest }>(`/police-promotions/bot/requests/${requestId}/evaluation`, input); return data.request; }
  ...
  async decidePolicePromotionRequest(requestId: string, input: { actorId: string; actorName: string; actorRoleIds?: string[]; approvalReason?: string | null; result: "approved" | "rejected" | "quarantined" }) { const { data } = await this.http.post<{ request: PolicePromotionRequest }>(`/police-promotions/bot/requests/${requestId}/decision`, input); return data.request; }
  ...
  async getPoliceRankUpSettings(guildId: string) { const { data } = await this.http.get<{ settings: PoliceRankUpSettings }>(`/police-rank-up/bot/${guildId}/settings`); return data.settings; }
  async savePoliceRankUpSettings(guildId: string, input: Partial<PoliceRankUpSettings>, actorId?: string | null) { const { data } = await this.http.patch<{ settings: PoliceRankUpSettings }>(`/police-rank-up/bot/${guildId}/settings`, input, { headers: actorId ? { "x-actor-id": actorId } : undefined }); return data.settings; }
  async createPoliceRankUpRequest(input: { currentRankId?: string | null; currentRoleId?: string | null; guildId: string; requestedRankId: string; temporaryChannelId?: string | null; userDisplayName: string; userId: string; username: string }) { const { data } = await this.http.post<{ request: PoliceRankUpRequest }>("/police-rank-up/bot/requests", input); return data.request; }
  async getPoliceRankUpRequest(requestId: string) { const { data } = await this.http.get<{ request: PoliceRankUpRequest }>(`/police-rank-up/bot/requests/${requestId}`); return data.request; }
  async getPoliceRankUpRequestByChannel(channelId: string) { const { data } = await this.http.get<{ request: PoliceRankUpRequest | null }>(`/police-rank-up/bot/channels/${channelId}/request`); return data.request; }
  async updatePoliceRankUpRequestChannel(requestId: string, input: { messageId?: string | null; temporaryChannelId?: string | null }) { const { data } = await this.http.patch<{ request: PoliceRankUpRequest }>(`/police-rank-up/bot/requests/${requestId}/channel`, input); return data.request; }
  async decidePoliceRankUpRequest(requestId: string, input: { actorId: string; actorName?: string | null; errorReason?: string | null; reason?: string | null; result: "approved" | "rejected" | "cancelled" | "error" }) { const { data } = await this.http.post<{ request: PoliceRankUpRequest }>(`/police-rank-up/bot/requests/${requestId}/decision`, input); return data.request; }
  async createPoliceRankUpLog(input: { action: string; actorId?: string | null; actorName?: string | null; guildId: string; metadata?: Record<string, unknown>; requestId?: string | null }) { await this.http.post("/police-rank-up/bot/logs", input); }
  ...
  async getVisibleMessageUser(guildId: string, userId: string) { const { data } = await this.http.get<{ user: VisibleMessageUser | null }>(`/visible-message/bot/${guildId}/users/${userId}`); return data.user; }
  async setVisibleMessageUserEnabled(guildId: string, userId: string, enabled: boolean, actorId?: string | null) { const { data } = await this.http.patch<{ user: VisibleMessageUser }>(`/visible-message/bot/${guildId}/users/${userId}/status`, { enabled }, { headers: actorId ? { "x-actor-id": actorId } : undefined }); return data.user; }
